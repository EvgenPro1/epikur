...
const initialState = {gender:null, age:null, height:null, weight:null, activity:null, goal:null, kkal:null, foodValue:''}
...
const Calc = () => {
    ...
    const handleChangeActivity = ({target:{value}}) => {
        setState({...state, activity: activityCoefficient[value-1]})
    }
    ...
    const getResult = () => {
        const newResult = 10*state.weight + 6.25*state.height - 5*state.age + state.gender - state.goal
...
return (
    ...
    <div className="calculator">
        <h3 className="calculator-header text-center pl-5 pr-5 mb-0">
            {/*<span>*/}
                Скільки білків мені потрібно на добу?
            {/*</span>*/}
        </h3>
        <div className="calculator-body calculator-body-top justify-content-center align-items-center d-flex flex-column p-2">
            <div className="d-flex flex-row align-items-center">

                <label className="d-flex bold">Стать</label>
                <div className="d-flex ">
                    <div className="d-flex wrapper flex-column justify-content-center m-3">
                        <label className="pointer d-flex align-items-center flex-column">
                            <input type="radio" name="gender" defaultChecked className='' onClick={()=>genderChecked(5)}/>
                    Чоловіча</label>
                </div>
                    <div className="d-flex wrapper flex-column justify-content-center">

                    <label className="pointer Calculator__radio d-flex align-items-center flex-column">
                        <input type="radio" name="gender" onClick={()=>genderChecked(-161)}/>
                        Жіноча</label>
                </div></div>
            </div>
            <div className="d-flex flex__nowrap">
                <div className="calc-input-block d-inline-flex ">
                    <label className="Calculator__label bold">Вік
                    <input type="number" min="5" className='calculator-input d-block'
                           value={state.age}
                           onChange={(e)=>enterChars(e,'age')}/></label>
                </div>
                <div className="calc-input-block d-inline-flex">
                    <label className="Calculator__label bold">Зріст (см)
                    <input type="number" min="50" max="220" className='calculator-input d-block'
                           value={state.height}
                           onChange={(e)=>enterChars(e,'height')}/></label>
                </div>
                <div className="calc-input-block d-inline-flex">
                    <label className="Calculator__label bold">Вага (кг)
                    <input type="number" max="200" value={state.weight} className='calculator-input d-block'
                           onChange={(e)=>enterChars(e,'weight')}/></label>
                </div>
            </div>

                <div className='d-flex align-items-center flex-column'>
                    <label className="bold">Спосіб життя</label>
                    <select className='pointer select-border' onChange={(e)=> handleChangeActivity(e)}>
                    <option className='rad123' value="1">Розумова дiяльнiсть</option>
                    <option className='rad123' value="2">Легке фiзичне навантаження</option>
                    <option className='rad123' value="3">Значне фiзичне навантаження</option>
                    <option className='rad123' value="4">Важка фiзична праця</option>
                    <option className='rad123' value="5">Особливо важка фізична праця</option>
                </select>
                </div>
                </div>
<div className='calculator-body calculator-body-bottom justify-content-center align-items-center d-flex flex-column p-2'>
            <div className='d-flex align-items-center flex-column mt-3'>
                <label className="Calculator__label bold">Мета</label>
                <div className="d-flex flex-row align-items-center">
                    <div className="d-flex wrapper flex-column justify-content-center goal-item-block">
                        <label className="pointer d-flex align-items-center flex-column">
                        <input type="radio" name="goal" value="1" id="calc-goal-lose_weight"
                        onChange={()=>setGoal(500)}/>
                            Скинути вагу</label></div>
                    <div className="d-flex wrapper flex-column justify-content-center goal-item-block">
                        <label className="pointer d-flex align-items-center flex-column">
                        <input type="radio" name="goal" value="2" id="calc-goal-maintaining_weight"
                               onChange={()=>setGoal(250)}/>
                            Підтримувати вагу</label></div>
                </div>
            </div>
             <div className="flex">

                 <span className="closing-button btn-test" onClick={getResult}>
            ...
            <div className="flex_2">
                <div className="Calculator__result">
                    {state.kkal||''}
                    <small>грама білка в день</small></div>
                <div className="Calculator__result rovno">&nbsp;=&nbsp;</div>
                <div className="Calculator__result">
                    XXX
                    <small>грам курятини</small></div>
            </div>
        </div>
    </div>
    </div>
